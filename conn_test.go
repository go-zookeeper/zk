...
func TestRecurringReAuthHang(t *testing.T) {
	WithTestCluster(t, 3, ioutil.Discard, ioutil.Discard, func(t *testing.T, ts *TestCluster) {
		WithConnectAll(t, ts, func(t *testing.T, c *Conn, ech <-chan Event) {
			ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
			defer cancel()

			waitForSession(ctx, ech)
			// Add auth.
			c.AddAuth("digest", []byte("test:test"))
