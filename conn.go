...
type Conn struct {
        ...
        server         string     // remember the ip address/port of the current server
        hostname       string
        conn           net.Conn
...
func Connect(servers []string, sessionTimeout time.Duration, options ...connOption) (*Conn, <-chan Event, error) {
        ...
        conn := &Conn{
                ...
                saslConfig:     &SASLConfig{SASLType: NONESASL},
        }
...
func WithSASL(saslConfig *SASLConfig) connOption {
        return func(c *Conn) {
...
func (c *Conn) connect() error {
        ...
        for {
                ...
                if zkConn, err := c.dialer("tcp", c.Server(), c.connectTimeout); err == nil {
                        c.conn = zkConn
                        c.setState(StateConnected)
                        log.Infof("success to connect to %s", c.Server())
                        return nil
                } else {
                        log.Errorf("failed to connect to %s: err: %s", c.Server(), err.Error())
                }
...
func (c *Conn) sendRequestEx(
        ...
        case resp = <-resChan:
        case <-c.closeChan:
                log.Debugf("recv routine closed")
                return false, nil
        case <-c.shouldQuit:
                log.Debugf("should quit")
                return false, nil
        case <-ctx.Done():
...
func (c *Conn) loop(ctx context.Context) {
        ...
        for {
                ...
                case err == ErrSessionExpired:
                        log.Errorf("failed to authenticate, err: %s", err.Error())
                        c.invalidateWatches(err)
                case err != nil && c.conn != nil:
                        log.Errorf("failed to authenticate, err: %s", err.Error())
                        c.conn.Close()
                case err == nil:
                        log.Debugf("authenticated: session id: %d, session timeout: %d", c.SessionID(), c.sessionTimeoutMs)
                        ...
                        go func() {
                                ...
                                if err := resendZkAuth(ctx, c); err != nil {
                                        log.Errorf("failed to resend auth creds, err: %s", err.Error())
                                        return
                                }

                                if err := c.sendLoop(); err != nil {
                                        log.Errorf("send loop is terminated, err: %s", err.Error())
                                }
                        }()
                        ...
                        go func() {
                                ...
                                if err != io.EOF {
                                        log.Errorf("recv loop is terminated, err: %v", err)
                                }
                                if err == nil {
...
func (c *Conn) sendSetWatches() {
        ...
        go func() {
                ...
                for _, req := range reqs {
                        ...
                        if err != nil {
                                log.Warnf("failed to set previous watches, err: %s", err.Error())
                                break
                        }
                }
        }()
...
func (c *Conn) recvLoop(conn net.Conn) error {
        ...
        for {
                ...
                if res.Xid == -1 {
                        ...
                        log.Debugf("xid: %d < 0 but not ping or watcher event", res.Xid)
                } else {
                        if res.Zxid > 0 {
                                c.lastZxid = res.Zxid
                        }
                        ...
                        if !ok {
                                log.Errorf("response for unknown request with xid %d", res.Xid)
                        } else {
                                if res.Err != 0 {
                                        err = res.Err.toError()
                                } else {
...
func (c *Conn) queueRequest(opcode int32, req interface{}, res interface{}, recvFunc func(*request, *responseHeader, error)) <-chan response {
        ...
        case opClose:
                ...
                case <-time.After(c.connectTimeout * 2):
                        log.Debugf("gave up trying to send opClose to server")
...
func (c *Conn) CreateProtectedEphemeralSequential(path string, data []byte, acl []ACL) (string, error) {
        if err := validatePath(path, true); err != nil {
                log.Errorf("failed to validate path: %s, err: %s", path, err.Error())
                return "", err
        }
        ...
        if err != nil {
                log.Errorf("failed to get guid, err: %s", err.Error())
                return "", err
        }
...
func resendZkAuth(ctx context.Context, c *Conn) error {
        ...
        log.Debugf("re-submitting `%d` credentials after reconnect", len(c.creds))
        if shouldCancel() {
                return nil
        }
        ...
        case KERBEROS:
                if krbCli, err := NewKerberosClient(c.saslConfig.KRBConfig); err != nil {
                        log.Fatalf("failed to create kerberos client, err: %s", err.Error())
                        return fmt.Errorf("failed to create kerberos client, err: %s", err.Error())
                } else {
                        var krbAuth = &GSSAPIKerberosAuth{
                                krbCli: krbCli,
                                Config: c.saslConfig.KRBConfig,
                        }
                        if err := krbAuth.Authorize(ctx, c); err != nil {
                                log.Errorf("kerberos auth failed, err: %s", err.Error())
                                return err
                        } else {
                                log.Debugf("kerberos auth successfully")
                        }
                }
        }
        ...
        for _, cred := range c.creds {
                ...
                if shouldContinue, err := c.sendRequestEx(
                        ctx,
                        opSetAuth,
                        &setAuthRequest{
                                Type:   0,
                        ...
                        log.Errorf("failed to send cred: %+v auth, err: %s", cred, err.Error())
                        if shouldContinue {
                                continue
                        }
                        return err
